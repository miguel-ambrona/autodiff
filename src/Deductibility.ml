  ...
  | F(name,_,_,(left,right),inv,_,_) ->
     ...
     |> L.fold_left ~init:[]
            ~f:(fun list (name,inv,n,m) ->
              begin match L.find list ~f:(fun (name',_,_,_) -> S.equal name name') with
              | None -> list @ [(name,inv,n,m)]
              | Some (_,inv',n',m') ->
                 if equal_bool inv inv' && n = n' && m = m' then list
    ...
    | f :: rest ->
       ...
       | F(name,i,m,(left,right),inv,_,a) ->
          if not inv then aux output rest
              ...
              L.fold_left rest
                  ...
                  ~f:(fun (idxs,others) f' ->
                    ...
                    | F(name',i',m',(left',right'),inv',_,_) ->
                       if S.equal name name' && equal_mode m m' && inv' &&
            ...
            if (compare_lists (range 0 (L.length right)) (L.sort idxs ~compare:cmp) ~compare:cmp ) = 0 then
...
let fun_deduce frame expr =
  let all_funs = L.map frame.frame_sigma ~f:get_all_funs |> L.concat |> L.dedup_and_sort ~compare:compare_expr in
  let clusters = cluster_funs all_funs in
  let known_constants =
    L.map (expr :: frame.frame_sigma) ~f:atoms |> L.concat
    |> L.map ~f:(function | Const a -> a | _ -> assert false)
    |> L.dedup_and_sort ~compare:S.compare
...
let pp_frame _fmt f =
  F.printf "{%a} %a\n" (pp_list ", " pp_expr) f.frame_sigma (pp_list ", " pp_string) f.frame_names;

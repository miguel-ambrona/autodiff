...
let generate_code2 ~oname ~atoms relations =
  let function_subterms_all =
    ...
    |> L.concat
    |> L.filter ~f:(function | F(name,_,_,(_,_),_,_,_) -> not(S.equal name oname) | _ -> false)
    |> L.dedup_and_sort ~compare:compare_expr
  ...
  L.fold_left function_subterms_all
      ...
      ~f:(fun b function_subterms ->
        if b then b
          ...
          Printf.fprintf out_file "is_universal{%s\n  distinguisher :=\n" file_code;
          let colors = !use_colors in
          use_colors := false;
          let old_xor_symbol = !xor_symbol in
          xor_symbol := "+";
          L.iter atoms ~f:(fun n -> Printf.fprintf out_file "    %s <-$ {0,1}^n;\n" n);
          L.iter assignments ~f:(fun (s,name) -> Printf.fprintf out_file "    %s <- %s;\n" name (string_of_expr s)); Printf.fprintf out_file "\n";
          L.iter relations ~f:(fun r -> Printf.fprintf out_file "    assert %s = 0;\n" (string_of_expr (replace r)));
          Printf.fprintf out_file "    return 1;\n";
...
let generate_distinguishers_code ~oname (sol, relations, (variable_tuples, keys)) =
  let inputs = L.filter (Core_kernel.Map.to_alist sol) ~f:(function | (n,_) -> (S.slice n 0 1) = "X") in
  let atoms = L.map inputs ~f:(fun (_,e) -> vars e ) |> L.concat |> L.dedup_and_sort ~compare:S.compare in
  ...
  L.fold_left function_subterms_all
      ...
      ~f:(fun b function_subterms ->
        if b then b
          ...
          let old_xor_symbol = !xor_symbol in
          xor_symbol := "+";
          L.iter atoms ~f:(fun n -> Printf.fprintf out_file "    %s <-$ {0,1}^n;\n" n); Printf.fprintf out_file "\n";
          L.iter assignments ~f:(fun (s,name) -> Printf.fprintf out_file "    %s <- %s;\n" name (string_of_expr s)); Printf.fprintf out_file "\n";
          L.iter inputs ~f:(fun (n,e) -> Printf.fprintf out_file "    %s = %s;\n" n (string_of_expr (replace e))); Printf.fprintf out_file "\n";
          let sep = if keys = [] then "" else "; " in
          L.iter variable_tuples
                 ~f:(fun (ins,outs) ->
                   L.iter (range 0 (L.length outs))
                          ~f:(fun i -> Printf.fprintf out_file "    %s = %s_%d(%s%s%s);\n" (string_of_expr (L.nth_exn outs i)) oname (i+1)
          ...
          let replace2 = function | Var n -> (begin match Core_kernel.Map.find sol n with | Some e -> e | None -> Var n |> simplify_expr end) | _ -> assert false in
          ...
          L.iter (range 0 (n_vars - 1))
                 ~f:(fun i -> L.iter (range (i+1) (n_vars - 1))
                                     ~f:(fun j ->
                                       ...
                                       if equal_expr (replace2 vi) (replace2 vj) then Printf.fprintf out_file "    assert %s = %s;\n" (string_of_expr vi) (string_of_expr vj)
                                       else Printf.fprintf out_file "    assert %s <> %s;\n" (string_of_expr vi) (string_of_expr vj)
                                     )
                    );
          Printf.fprintf out_file "    return 1;\n";
          Printf.fprintf out_file "  .\n\n}.\n";

...
let rec freeze_variables = function
  | Var n -> Const ("%" ^ n)
  | Zero -> Zero
  | Const n -> assert (S.compare (S.slice n 0 1) "%" <> 0); Const n
...
let simplify_frame_for_deduction f =
  let new_names =
    L.filter f.frame_names
       ~f:(fun n -> not (L.exists f.frame_sigma ~f:(function | Const n' -> S.equal n n' | _ -> false)))
  in
...
let deduce_with_vars frame e =
  ...
  let sts = if L.length (vars e) = 0 then sts else L.tl_exn sts in
  ...
  |> L.filter
       ~f:(fun u ->
         ...
         assert (not(L.exists constants ~f:(fun n -> S.equal (S.slice n 0 1) "%")));

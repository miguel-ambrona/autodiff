...
let string_of_assignment_web (x,e) =
    "& \\quad \\mathsf{ " ^ (color "red" x) ^ " \\: \\rightarrow \\: " ^ (string_of_expr e) ^ " }"

let string_of_unifier_web unifier =
  (string_of_list ";@  " string_of_assignment_web (String.Map.to_alist unifier)) ^ "@"
...
let pp_unifier_web _ unifier = F.printf "%s" (string_of_unifier_web unifier)
...
let is_unconstrained ~var expr =
  let rec aux top = function
    | Const _ | Zero -> true
    | Var x -> if S.equal x var then top else true
    | F(_,_,_,(left,right),_,_,_) ->
...
let find_unconstrained_var exprs =
  let variables = L.map exprs ~f:vars |> L.concat |> L.dedup_and_sort ~compare:S.compare in
...
let similar_fun fun1 fun2 =
  ...
  | F(n1,i1,m1,(left1,right1),inv1,_,_), F(n2,i2,m2,(left2,right2),inv2,_,_) ->
     if S.equal n1 n2 && i1 = i2 && equal_mode m1 m2 then
       (assert ((L.length left1) = (L.length left2) && (L.length right1) = (L.length right2));
        assert (equal_bool inv1 inv2);
        true
       )
     else false
  | _ -> false
...
let unify_liu ~stopTime unifp =
  let rec aux sigma equalized unifp =

    let t' = getTime () in
    match stopTime with
    | Some t when (Float.compare t' t) > 0 -> None
    | _ ->
...
let unify ?(timeout = None) unifp =

  let stopTime = match timeout with None -> None | Some t -> Some (t +. getTime()) in
  let rec aux unifp =
    let equalities = L.map unifp.unif_equalities ~f:simplify_expr
                     |> L.dedup_and_sort ~compare:compare_expr
                     |> L.filter ~f:(function | Zero -> false | _ -> true)
    ...
    if L.exists without_vars ~f:(fun e -> not (equal_expr e Zero)) then None
      ...
      | None -> unify_liu ~stopTime { unifp with unif_equalities = with_vars }

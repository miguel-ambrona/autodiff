...
let rec get_funs_with_arity expr =
  ...
  | F(name,_,_,(left,right),inv,_,_) ->
     ...
     (name,inv,L.length left, L.length right, L.length right) :: other_funs
  end
  |> L.fold_left
       ~init:[]
       ~f:(fun list (name,inv,n,m,m2) ->
         begin match L.find list ~f:(fun (name',_,_,_,_) -> S.equal name name') with
         | None -> list @ [(name,inv,n,m,m2)]
         | Some (_,inv',n',m',m2') ->
            if equal_bool inv inv' && n = n' && m = m' && m2 = m2' then list
...
let all_contexts_of_size_at_most_n n atoms _funs =
  ...
  let rec compute_contexts k =
    if k = 1 then atoms
      ...
      (*
      let calculated =
        L.map _funs ~f:(fun (name,inv,d1,d2,d3) ->
           L.map (all_k_lists_sum_n (d1+d2) (k-1))
             ~f:(fun l ->
               ...
               L.map combinations
                  ~f:(fun comb ->
                    if L.exists comb ~f:(fun e -> equal_sig (sign e) F_sig) then []
                    else
                      let left,right = split_list_by_idx d1 comb in
                      if inv then
                        (L.map (range 0 d3) ~f:(fun i -> F(name,i,POS,(left,right),inv,false,None))) @
                          (L.map (range 0 d3) ~f:(fun i -> F(name,i,NEG,(left,right),inv,false,None)))
                      else
                        L.map (range 0 d3) ~f:(fun i -> F(name,i,POS,(left,right),inv,false,None))
                  )
               |> L.concat
      ...
      *)
      ...
      (match Map.find !all_contexts k with
       | None -> all_contexts := Map.add_exn !all_contexts ~key:k ~data:contexts;
       | Some l -> all_contexts := Map.add_exn (Map.remove !all_contexts k) ~key:k ~data:(l);
      );
      contexts
  in
...
let all_contexts_of_size_at_most_n_2 n atoms funs =
  ...
  let rec compute_contexts k =
    if k = 1 then atoms
      ...
      in
      let calculated =
        L.map funs ~f:(fun (name,inv,d1,d2,d3) ->
           L.map (all_k_lists_sum_n (d1+d2) (k-1))
             ~f:(fun l ->
      ...
      (match Map.find !all_contexts k with
       | None -> all_contexts := Map.add_exn !all_contexts ~key:k ~data:contexts;
       | Some l -> all_contexts := Map.add_exn (Map.remove !all_contexts k) ~key:k ~data:(l); (*contexts @ l *)
      );
      contexts
  in
  let rec asc_iter k =
...
let heuristic1 ~primitive ~k ~nin ~nout ~oname n_inputs rel1 rel2 =
  let var_name v i = (string_of_int v) ^ (S.make i '\'') in
  let variable_tuples  =
    L.map (range 0 n_inputs)
          ~f:(fun i ->
            L.map (range 0 nin) ~f:(fun v -> Var ("X"^(var_name v i))),
            L.map (range 0 nout) ~f:(fun v -> Var ("Y"^(var_name v i)))
          )
  in
  ...
  let relations =
    [L.fold_left (range 0 (n_inputs*nin)) ~init:rel1 ~f:(fun rel i -> replace_expr rel ~old:(Var (string_of_int i)) ~by:(L.nth_exn input_vars i));
     L.fold_left (range 0 (n_inputs*nout)) ~init:rel2 ~f:(fun rel i -> replace_expr rel ~old:(Var (string_of_int i)) ~by:(L.nth_exn output_vars i))]
  in
  let inequalities =
    L.fold_left (range 0 (n_inputs*nout))
        ~init:rel2
        ~f:(fun rel j ->
  ...
  let sol = unify ~timeout:(Some 0.1) u in (* Timeout if not solved after 100 ms *)
  sol, relations, (variable_tuples,keys)

let heuristic ~primitive ~k ~nin ~nout ~oname ~context_size =
  (* We put them inside a fake F to analyze them at the same time *)
  let exprs = primitive (list_repeat (Var ".") k) (list_repeat (Var ".") nin) in
  let funs =
    get_funs_with_arity (F("fakename", 0, POS, (exprs, []), false, false, None))
    |> L.tl_exn
  in
  let aux n_inputs =
    let atoms = L.map (range 0 (n_inputs*nin)) ~f:(fun i -> Var (string_of_int i)) in
               ...
               ~f:(fun c ->
                 ...
                 L.fold_left (range 0 n_inputs)
                    ...
                    ~f:(fun b i -> if not b then b
                                   else if not (L.exists (range (nin*i) (nin*(i+1))) ~f:(fun j -> L.mem c_vars j ~equal:(=))) then false
                                   else true
                       )
               )
    in
            ...
            L.fold_left contexts
              ...
              ~f:(fun solved rel2 ->
                  begin match solved with
                    | Some _ -> solved
                    | None ->
                      begin match heuristic1 ~primitive ~k ~nout ~nin ~oname n_inputs rel1 rel2 with
  ...
  in
  L.fold_left (range 3 7)
      ~init:None
      ~f:(fun solved i -> match solved with Some _ -> solved | None -> aux i)

type attack = Indiff | INDCCA | INDCPA

let heuristic2 ~primitive ~inv_primitive ~attack_type ~k ~nin ~nout ~oname ~atoms ~size ~sol_number =
  (* We put them inside a fake F to analyze them at the same time *)
  let exprs = primitive (list_repeat (Var ".") k) (list_repeat (Var ".") nin) in
  let funs =
  ...
  let funs =
    funs @
      (match attack_type with
       | INDCPA -> [(oname, false, k, nin, nout)]
       | _ -> [(oname, (match inv_primitive with | None -> false | Some _ -> true), k, nin, nout)]
      )
  in
  let contexts =
    all_contexts_of_size_at_most_n_2 size (L.map atoms ~f:(fun s -> Const s)) funs
    |> L.filter ~f:(fun c -> not(equal_sig (sign c) XOR_sig))
    |> dedup_preserving_order ~equal:equal_expr
  in
  ...
  let rec convert_to_real_world expr =
    ...
    | F(name,i,m,(left,right),invertible,b,a) ->
       ...
       if S.equal name oname then
         let prim = if equal_mode m POS then primitive else
                      (assert invertible;
         ...
         F(name,i,m,(left',right'),invertible,b,a)
  in
  let rec get_terms = function | Zero -> [] | XOR(exprs) -> L.map exprs ~f:get_terms |> L.concat | e -> [e] |> L.dedup_and_sort ~compare:compare_expr in
  let pairs =
  ...
  let terms = L.fold_left pairs ~init:[] ~f:(fun l (_,_,ts) -> l @ ts) |> L.dedup_and_sort ~compare:compare_expr in
  let linear_system =
  ...
  | None -> None
  | Some _matrix ->
    let solution = Static_Equivalence.linalg_nth_solution ~nth:sol_number _matrix in
...
let iterative_heuristic2 ~primitive ~inv_primitive ~attack_type ~k ~nin ~nout ~oname =

  let max_atoms = ["b"] (*"c"; "d"; "e"; "f"; "g"] in*) in
  (* First try with small sizes, then jump to 6 *)
  let sizes = [2; 3; 6] in
             ...
             ~f:(fun b size ->
               ...
               | None ->
                  let rec iterate sol_number =
                    match heuristic2 ~primitive ~inv_primitive ~attack_type ~k ~nin ~nout ~oname ~atoms ~size ~sol_number with
                    | None -> None
                    | Some (sol, valid) ->

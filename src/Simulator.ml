         ...
         let is_valid_unifier = L.fold_left additional
              ...
              ~f:(fun b (v1,v2) ->
                if not b then b
                  ...
                  let forbidden = consts (prepare v1) |> L.filter ~f:(fun s -> S.equal (S.slice s 0 1) "%") in
                  let f = { frame_names = frame1.frame_names @ forbidden; frame_sigma = L.map frame1.frame_sigma ~f:prepare; } in

                  begin match deduce f (prepare v1) with

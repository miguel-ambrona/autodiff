...
let equal_is_eq eq1 eq2 =
  match eq1, eq2 with | Eq, Eq | Ineq, Ineq -> true | _ -> false
...
let check_arguments_consistency (name,d_left,d_right) =
  ...
  | None -> fun_args := Map.add_exn !fun_args ~key:name ~data:(d_left,d_right)
  | Some (l,r) ->
  ...
  let rec aux = function
    ...
    | F(name, i, mode, (left,right), invertible, blocked, alias) ->
      (*check_arguments_consistency (name, L.length left, L.length right);*)
       begin match L.find rounds ~f:(fun (n,_) -> S.equal n name) with
       | None ->
          if L.exists os ~f:(fun (n,inv,_,_,_) -> S.equal n name && equal_mode inv mode) then
       ...
       | Some (_,invertible) ->
          if not invertible && equal_mode mode NEG then failwith ("Round function " ^ name ^ " is not invertible")
          else
            let left' = L.map left ~f:aux in
            let right' = L.map right ~f:aux in
            F(name, i, mode, (left', right'), invertible, blocked, alias)
...
let check_oracle_inverses oracles =
  L.iter oracles
     ~f:(fun (name,m,k,_,exprs) ->
       ...
       match L.filter oracles ~f:(fun (name',m',k',_,_) -> if S.equal name name' && not(equal_mode m m') then (assert (k = k'); true) else false) with
       | [] -> ()
       | (_,_,_,_,exprs') :: [] ->
          ...
          L.iter (range 0 (L.length exprs))
                 ~f:(fun i ->
                   ...
                   in
                   if equal_expr (L.nth_exn inputs i) (simplify_expr this_expr) then ()
                   else
...
let rec fix_invertibility ~is_oracle_invertible name = function
  | F(n',i,mode,(left,right),_,blocked,alias) when S.equal n' name ->
    let left' = L.map left ~f:(fix_invertibility ~is_oracle_invertible name) in
    let right' = L.map right ~f:(fix_invertibility ~is_oracle_invertible name) in
    F(n',i,mode,(left',right'),is_oracle_invertible,blocked,alias)

  | XOR (list) -> XOR(L.map list ~f:(fix_invertibility ~is_oracle_invertible name))
  | _ as e -> e
...
let apply_oracles_to_expr ~real oracles expr =
  let rec aux = function
    | Zero | Const _ | Var _ as e -> e
    | XOR (list) -> XOR(L.map list ~f:aux )
    | F(name, i, mode, (left,right),_,blocked,alias) ->
      (*check_arguments_consistency (name, L.length left, L.length right);*)
       let left' = L.map left ~f:aux in
       let right' = L.map right ~f:aux in
       let arguments = left' @ right' in
       let is_oracle_invertible = some (L.find oracles ~f:(fun (n',m',_,_,_) -> S.equal n' name && equal_mode m' NEG)) in
       begin match L.find oracles ~f:(fun (n',m',_,_,_) -> S.equal n' name && equal_mode m' mode) with
       | None -> F(name,i,mode,(left',right'),is_oracle_invertible,blocked,alias) (* This comes from a round function *)
       | Some (_,_,_,new_alias,outputs) ->
          if real then
            ...
            |> (fix_invertibility name ~is_oracle_invertible)
          else
            let invertible =
              begin match L.find oracles ~f:(fun (n',m',_,_,_) -> S.equal n' name && equal_mode m' NEG) with
              | Some _ -> true
              | None -> if equal_mode mode NEG then failwith ("The inverse of oracle " ^ name ^ " is not defined") else false
              end
            in
...
let rec contains_round rs = function
  ...
  | F(name,_,_,(left,right),_,_,_) ->
     check_arguments_consistency (name, L.length left, L.length right);
     if L.exists rs ~f:(fun (n,_) -> S.equal n name) then true else L.exists (left@right) ~f:(contains_round rs)
  ...
  let rec aux rep sim_rep knowledge system = function
    | DistProgCmd(cmd, p) ->
       ...
       | DistCmdSimQuery (v,n,m,i,args) ->
          ...
          let invertible =
            begin match L.find rs ~f:(fun (n',_) -> S.equal n n') with
            | Some (_,inv) -> inv
            | None -> failwith ("Round function " ^ n ^ " is not defined")
            end
          in
          let () = if not invertible && equal_mode m NEG then failwith ("Round function " ^ n ^ " is not invertible") else () in
          let e = F(n,i,m,([],L.map args ~f:(apply_replacement rep)), invertible, false, None) in
          var_counter := !var_counter + 1;
          let e' = Var ("V" ^ (string_of_int !var_counter)) in
          aux (rep @ [(v,e)]) (sim_rep @ [(v,e')]) (knowledge @ [(e',n,i,m,L.map args ~f:(fun e -> apply_replacement sim_rep e |> apply_oracles_to_expr ~real:false oracles))]) system p
       | DistCmdAssert(is_eq, e) ->
          if contains_round rs e then
            ...
            if (equal_expr e' Zero && equal_is_eq is_eq Ineq) || (not (equal_expr e' Zero) && equal_is_eq is_eq Eq) then
...
let equivalent_branches ~k b1 b2 =
  if k >= (L.length b1) || k >= (L.length b2) then false
    ...
    if L.exists (L.zip_exn b1_k b2_k) ~f:(fun ((_,n1,i1,m1,_),(_,n2,i2,m2,_)) -> not (S.equal n1 n2) || i1 <> i2 || not(equal_mode m1 m2)) then false
    else
      ...
      let forbidden_names = L.fold_left (frame1_elements @ frame2_elements) ~init:[] ~f:(fun l e -> l @ (consts e)) |> L.dedup_and_sort ~compare:S.compare in
      assert (L.length frame1_elements = L.length frame2_elements);
      assert (not (L.exists forbidden_names ~f:(fun n -> S.equal "%" (S.slice n 0 1))));

      let containsVars = ref false in
      let rec f = function
        | Zero | Const _  as e -> e
        | Var n -> containsVars := true; Const ("%" ^ n)
      ...
      (* If the system is not statically equivalent, but it contains variables, we have to throw a warning *)
      (
        if !containsVars && b then
          (F.printf "WARNING: Static equivalent with variables resulted in 'inequivalence', our methods may not be sound in this case\n\n";
           F.print_flush())
        else ()
      );
      b
...
let simplify_frame known_vars frame =
  ...
  let consts = L.map known_vars ~f:consts |> L.concat |> L.dedup_and_sort ~compare:S.compare in

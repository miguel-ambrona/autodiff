...
let analyze_if_it_is_universal ?(print = true) (rs, os, program) =
  ...
  let variables = L.map (u.unif_equalities @ u.unif_inequalities) ~f:vars |> L.concat |> L.dedup_and_sort ~compare:S.compare in
  ...
  | None ->
     if print then
       (F.printf "There does not exist a simulator for the distinguisher\n\n";
        print_time None t1 t2;  F.printf "\n")
     else ();
     true
  | Some sol ->
     if print then
...
let process_command cmd =
  ...
  | Unify u ->
     ...
     L.iter solutions ~f:(fun s -> F.printf "%a\n" pp_unifier s);
  ...
  | Knowledge (es,fs) ->
     if L.length es <> L.length fs then failwith "The number of expressions does not match the number of frames"
       ...
       use_colors := false; (* Disable colors to measure expressions length *)

...
let string_of_mode = function
  | POS -> ""
  | NEG ->
     if !use_colors && not !web_interface then "\xE2\x81\xBB" ^ "\xC2\xB9"
     else "^{-1}"
...
let rec string_of_expr = function
  ...
  | F(name, i, mode, (left_input, right_input), inv, blocked, alias) ->
     let idx =
       if L.length right_input = 1 && inv then
         if not (i = 0) && inv then "_" ^ (string_of_int (i+1)) (* assert false *)
         else  ""
     ...
     (color (if blocked then "yellow" else if inv then "blue" else "none")
            (match alias with
             | None -> name ^ idx ^ (string_of_mode mode)
             | Some a -> a ^ idx
            )
     )
     ^ "("  ^ content ^ ")"
...
let check_inverse n i m left right =
  let rec aux k y = function
    ...
    | e :: rest ->
       begin match e with
       | F(n',i',m',(left',right'),_inv,_,_) ->
         (*if not inv then None
           else*)
            let y = if L.length y = 0 then right' else y in

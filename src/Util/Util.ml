...
let print_time channel t1 t2 =
  let seconds = t2 -. t1 in
  let (time, units) =
    if seconds < 1.0 then (1000.0 *. seconds, "ms")
    else if seconds < 60.0 then (seconds, "s")
    else if seconds < 3600.0 then (seconds /. 60.0, "min")
    else if seconds < 3600.0 *. 24.0 then (seconds /. 3600.0, "h")
    else (seconds /. (3600.0 *. 24.0), "days")
  in
  match channel with
  | None    -> F.printf "Solved in %F %s\n" ((ceil (1000.0 *. time)) /. 1000.0) units
  | Some oc -> Printf.fprintf oc "Solved in %F %s\n" ((ceil (1000.0 *. time)) /. 1000.0) units

let print_time_web t1 t2 =
  F.printf "&\\textsf{\\color{#DEFAFF}{Solved in %F ms.}}@" (ceil ((1000000.0 *. (t2 -. t1))) /. 1000.0)


let use_colors = ref true
let web_interface = ref false
...
let pp_bool _ b = if b then F.printf "1" else F.printf "0"
...
let rec compare_lists ~compare list1 list2 =
  let list1 = L.sort ~compare list1 in
  let list2 = L.sort ~compare list2 in
...
let equal_lists ~compare list1 list2 =
  (compare_lists ~compare list1 list2) = 0
...
let safe_sort list ~compare =
  ...
  let rec aux visited sorted = function
    ...
    | a :: rest ->
       if L.exists (visited @ rest) ~f:(fun a' -> compare a a' > 0) then aux (visited @ [a]) sorted rest
